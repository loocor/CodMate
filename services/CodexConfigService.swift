...
actor CodexConfigService {
    ...
    private static let notifyScript: String = #"""
#!/bin/zsh
# CodMate → macOS notification bridge for Codex CLI `notify` events
...
# Parse JSON and decide whether to notify. Output: title\tmessage\tgroup_id
...
try:
    ...
    # Fallback: show a generic notification with a snippet
...
PY
  if command -v terminal-notifier >/dev/null 2>&1; then
    ...
    # Use terminal-notifier only when we can attribute to CodMate; otherwise
    # fall back to osascript to avoid a default clickable action opening editors.
