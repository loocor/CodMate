...
final class CodMateTerminalView: LocalProcessTerminalView {
    ...
    private static func cleanCJKInterWordSpaces(in s: String) -> String {
        ...
        while i < scalars.count {
            ...
            if isStripSpace(u) {
                ...
                if prev >= 0 && next < scalars.count && isCJKOrFullwidth(scalars[prev]) && isCJKOrFullwidth(scalars[next]) {
    ...
    private func installKeyMonitorIfNeeded() {
        ...
        keyMonitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown]) { [weak self] event in
            ...
            let isReturnKey = (event.keyCode == 36 /* Return */) || (event.keyCode == 76 /* Keypad Enter */)
            if isReturnKey {
    ...
    private func pasteInlineImage(data: Data) {
        ...
        let st = "\u{001B}\\"     // ESC \
        let payload = "\(oscStart)1337;File=;inline=1;preserveAspectRatio=1;width=auto;height=auto:\(base64)\(st)"
