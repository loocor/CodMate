...
extension ContentView {
    ...
    func applyTaskAndChangeModifiers<V: View>(to view: V) -> some View {
        ...
        let v3 = v2.onChange(of: selection) { _, newSel in
            ...
            if let primary = selectionPrimaryId, !newSel.contains(primary) { selectionPrimaryId = newSel.first }
    ...
    func applyDialogsAndAlerts<V: View>(to view: V) -> some View {
        view
            .confirmationDialog(
                ...
                isPresented: Binding<Bool>(get: { confirmStopState != nil }, set: { if !$0 { confirmStopState = nil } }),
                titleVisibility: .visible
            ) {
                Button("Stop", role: .destructive) {
                    if let st = confirmStopState { stopEmbedded(forID: st.sessionId); confirmStopState = nil }
                }
            ...
            } message: { _ in Text("Session files will be moved to Trash and can be restored in Finder.") }
            .fileImporter(isPresented: $selectingSessionsRoot, allowedContentTypes: [.folder], allowsMultipleSelection: false) { result in
