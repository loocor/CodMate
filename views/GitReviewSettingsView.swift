...
struct GitReviewSettingsView: View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                Grid(alignment: .leading, horizontalSpacing: 16, verticalSpacing: 14) {
                        ...
                        HStack(spacing: 8) {
                            Picker("", selection: Binding(
                                ...
                                set: { newVal in
                                    ...
                                    modelId = models.contains(preferences.commitModelId ?? "") ? preferences.commitModelId : nil
                                }
                            )) {
                                Text("Auto").tag("(auto)")
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        Text("Optional preamble used before the diff when generating commit messages. Leave blank to use the built‑in prompt.")
    ...
    private func modelsForCurrentProvider() -> [String] {
        guard let pid = providerId, let p = providersList.first(where: { $0.id == pid }) else { return [] }
