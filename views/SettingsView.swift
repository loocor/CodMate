            ...
            VStack(alignment: .leading, spacing: 16) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("Claude Code Settings")
                        .font(.title2)
                        .fontWeight(.bold)
                    Text("Configure Claude Code runtime defaults and model alias mappings.")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                }
                ...
                settingsCard {
                    Grid(alignment: .leading, horizontalSpacing: 16, verticalSpacing: 12) {
                        GridRow {
                            ...
                            Picker("", selection: $claudeVM.activeProviderId) {
                                Text("(None)").tag(String?.none)
                                ForEach(claudeVM.providers, id: \.id) { provider in
                                    Text(provider.name?.isEmpty == false ? provider.name! : provider.id)
                                        .tag(String?(provider.id))
                                }
                            }
                            .labelsHidden()
                            .frame(maxWidth: .infinity, alignment: .trailing)
                            .onChange(of: claudeVM.activeProviderId) { _, _ in
                                Task { await claudeVM.applyActiveProvider() }
                            }
                        }
                        ...
                        GridRow {
                            VStack(alignment: .leading, spacing: 0) {
                                Text("Model Aliases").font(.subheadline).fontWeight(.medium)
                                Text("Map Claude Code aliases to vendor-specific models.")
                                    .font(.caption).foregroundStyle(.secondary)
                            }
                            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack {
                    Spacer()
                    Button("Save") { Task { await claudeVM.save() } }
                        .buttonStyle(.borderedProminent)
                }

                if let error = claudeVM.lastError, !error.isEmpty {
                    Text(error).foregroundStyle(.red).font(.caption)
                }
            }
        }
        .task { await claudeVM.loadAll() }
    }
